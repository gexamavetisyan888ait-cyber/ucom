...
export default function CardFirst() {
  ...
  return (
    <div className="mt-12 flex flex-wrap justify-between items-center gap-6">
      {cards.map(el => (
        <img
          key={el}
          src={el}
          alt=""
          className="h-[400px] w-[275px] rounded-2xl border border-gray-300 object-cover transition-opacity duration-300 hover:opacity-10"
        />
      ))}
    </div>
  );
